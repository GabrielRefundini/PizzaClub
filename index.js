...
if (window.ethereum) {
  ...
  web3.eth.getAccounts(function (err, accounts) {
    if (err != null) console.error("Aconteceu um erro: " + err);
    else if (accounts.length == 0)
      Swal.fire({
        icon: "error",
        title: "Oops...",
        text: "Voce não esta logado na MetaMask, logue antes de iniciar o jogo!",
      });
    else {
      console.log("Usuario esta logado na MetaMask");
      iniciaJogo();
    }
    var account;
    account = accounts[0];
  ...
  Swal.fire({
    icon: "error",
    title: "Oops, cade a MetaMask?...",
    text: "Navegador não suporta web 3, tente instalar a extensão da MetaMask primeiro!",
  });
...
function mint() {
  ...
  Swal.fire({
    title:
      "Paranbes! Voce ganhou um token Pizza, clique no botão RECEBER para obter seu token",
...
function mintNFT() {
  const uri_nft =
    "https://gateway.pinata.cloud/ipfs/QmWagcFamioPGMc3Ap3SMUF9xrPcwcMsfX5BzKut5aJMgo";

  contract.methods
    .safeMint(web3.eth.defaultAccount, uri_nft)
    .send({ from: web3.eth.defaultAccount })
    .then(function (tx) {
      console.log("Transaction: ", tx);
      Swal.fire("Mint realizado com sucesso!", "", "sucess");
    })
    .catch(function (e) {
      console.log("Error on transaction: ", e);
      Swal.fire("Erro ao realizar Mint!", "", "error");
    });
}
...
function saldoToken() {
  getsaldoToken().then(function (result) {
    const elem = document.getElementById("quantidadeTokens");
